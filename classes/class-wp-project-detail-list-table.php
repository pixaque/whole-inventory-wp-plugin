 ...
 * @see WP_List_Table
 */
class WP_Projects_Detail_List_Table extends WP_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$posts = $wpdb->get_results( "SELECT * FROM $table_name WHERE `projectid` = $projectId" );
	...
	public function display_rows( $posts = array(), $level = 0 ) {
		...
		if ( empty( $posts ) ) {
			...
			if(empty($search)){
				...
				$sql = $wpdb->prepare( "SELECT * FROM $table_name WHERE `projectid` = $projectId ORDER BY `id` $order LIMIT %d,%d", $offset, $per_page );
				...
				//$sql = $wpdb->prepare( "SELECT * FROM $table_name WHERE site_location LIKE = %s", $search );
				$sql = $wpdb->prepare( 
				"SELECT * FROM $table_name 
				WHERE (projectid LIKE %s OR billNo LIKE %s OR description LIKE %s OR orderTotal LIKE %s OR billdate LIKE %s) 
				AND `projectid` = %d
				ORDER BY `id` ASC", 
				$like, // projectid 
				$like, // billno
				$like, // description
				$like, // orderTotal
				$like, // Assuming billdate is a date, use it directly
				$projectId
				);
	...
	foreach ( $post as $result) {
		...
		$orderTotal[$i] = $result->orderTotal;
	...
	?>
		<tr>
			...
			<td id="actionWrapper">
				<a href="<?php echo "?page=wp_wer_pk_order_detail&order=".$result->id ?>"><?php echo $result->billNo ?></a>
				<div Class="actionDiv">
					<a href="javascript:void(0);" onClick="wer_pkEditFromTable(<?php echo $result->id ."," . $result->projectid ?>);">
						<span Class="dashicons-before dashicons-edit"></span><?php echo __('Edit', 'wer_pk') ?>
					</a>|
					<a href="javascript:void(0);" onClick="wer_pkDeleteFromTable(<?php echo $result->projectid . ',' . $result->id . ',\' ' . __('Are you sure?', 'wer_pk') . '\''; ?> );">
			...
			<td><?php echo $result->expenseType ?></td>
