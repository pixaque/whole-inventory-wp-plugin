<?php
/**
 * List Table API: WP_Projects_List_Table class
 *
 * @package WordPress
 ...
 * @see WP_List_Table
 */
class WP_Order_Detail_List_Table extends WP_List_Table {
	...
	public function __construct( $args = array() ) {
		...
		$posts = $wpdb->get_results( "SELECT * FROM $table_name WHERE `billid` = $orderId ORDER by 'id' ASC" );

		$this->user_projects_count = (int) COUNT( $posts );
	...
	public function prepare_items() {
		...
		if ( $this->hierarchical_display ) {
			...
			//$total_items = $this->user_projects_count;
	...
	protected function get_views() {
		...
		if ( $this->user_projects_count && $this->user_projects_count !== $total_posts ) {
			...
			$mine_inner_html = sprintf(
				...
				_nx(
					...
					'Mine <span class="count">(%s)</span>',
					$this->user_projects_count,
					'posts'
				),
				number_format_i18n( $this->user_projects_count )
			);
	...
	protected function extra_tablenav( $which ) {
		...
		if ( 'top' === $which ) {
			 ...
			 *                          'top' or 'bottom' for WP_Projects_List_Table,
	...
	public function display_rows( $posts = array(), $level = 0 ) {
		global $wp_query, $per_page, $wpdb, $search;
		...
		if ( empty( $posts ) ) {
			...
			$orderMethod = "DESC";
			if(isset($_GET['paged'])){
				$offset = ((int) $_GET['paged'] - 1) * $per_page;
			}

			if(isset($_GET['orderMethod'])){
				$orderMethod = $_GET['orderMethod'];
			}

			if(empty($search)){

				$sql = $wpdb->prepare( "SELECT * FROM $table_name WHERE `billid` = $orderId ORDER BY `id` $orderMethod LIMIT %d,%d", $offset, $per_page );
				...
				WHERE (supplierName LIKE %s OR materialsName LIKE %s OR totalPrice LIKE %s OR discount LIKE %s) 
				AND billid = %d 
				ORDER BY `id` ASC", 
				$like, // supplierName
				$like, // materialsName
	...
	private function _display_rows( $posts, $level = 0 ) {
		...
		$this->single_row( $posts, $level );

		?>
		<?php
		/*************
		<tr class="iedit author-self level-0 post-2629 type-post status-draft format-standard hentry category-uncategorized">
			<td><?php echo $post->id ?></td>
			<td><a href="#"><?php echo $post->site_name ?></a></td>
			<td><?php echo $post->site_size ?></td>
			<td><?php echo $post->site_location ?></td>
			<td><?php echo $post->start_date ?></td>
			<td><?php echo $post->status ? "On Hold" : "In Progress" ?></td>
			<td>
				<a href="?page=wp_wer_pk_blocks_projects&project=<?php echo $post->id ?>&action=edit">
					Edit
				</a> |
				<a id="deleteRecord" href="#">
					Delete
				</a>
			</td>
		</tr>
	...
	foreach ( $post as $result) {
		...
		$orderTotal[$i] = $result->totalPrice;
		$quantityTotal[$i] = $result->quantity;
		$gstTotal[$i] = $result->GST;
	...
	?>
		<tr>
			...
			<td id="actionWrapper">
				<?php echo $result->materialsName ?>
				<div Class="actionDiv">
					<a href="javascript:void(0);" onClick="wer_pkEditFromTable(<?php echo $result->billid .','. $result->id ?>);">
						<span Class="dashicons-before dashicons-edit"></span><?php echo __('Edit', 'wer_pk') ?>
					</a> |
					<a href="javascript:void(0);" onClick="wer_pkDeleteFromTable(<?php echo $result->billid . ',' . $result->id . ',\' ' . __('Are you sure?', 'wer_pk') . '\''; ?> );">
			...
			<td>
				<span class="dashicons-before <?php echo esc_attr(Settings::set_currency_symbol()); ?>"></span>: <?php echo $result->GST ?>
			</td>
			<td>
				<span class="dashicons-before <?php echo esc_attr(Settings::set_currency_symbol()); ?>"></span>:
		...
		<tr>
			...
			<td style="color: red; font-weight: bold;">
				<span class="dashicons-before <?php echo esc_attr(Settings::set_currency_symbol()); ?>"></span>:
				<?php echo number_format_i18n( array_sum($gstTotal), 2 ); ?>
			</td>
			<td style="color: red; font-weight: bold;">
				<span class="dashicons-before <?php echo esc_attr(Settings::set_currency_symbol()); ?>"></span>:
				<?php echo number_format_i18n( array_sum($orderTotal), 2 ); ?>
			</td>
