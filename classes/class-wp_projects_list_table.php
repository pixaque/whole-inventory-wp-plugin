 ...
 * @see WP_List_Table
 */
class WP_Projects_List_Table extends WP_List_Table {
	...
	public function no_items() {
		echo "Please add something to search. Sorry! Nothing found.";
		/*
	...
	public function display_rows_or_placeholder() {
		...
		if(!isset($_REQUEST['s'])){
			...
			$this->display_rows();
		} else if($this->get_column_count() == 0) {
	...
	public function display_rows( $posts = array(), $level = 0 ) {
		...
		if ( empty( $posts ) ) {
			...
			if(empty($search)){
				...
				$sql = $wpdb->prepare( "SELECT * FROM $table_name ORDER BY `start_date` $order LIMIT %d,%d", $offset, $per_page );
				...
				WHERE (site_name LIKE %s OR site_location LIKE %s OR start_date LIKE %s)", 
				$like, // site_name 
				$like, // site_location
				$like // Assuming start_date is a date, use it directly
				);
	...
	foreach ( $post as $result) {
		...
		<tr>
			...
			<td id="actionWrapper">
				<a href="<?php echo "?page=wp_wer_pk_project_detail&project=".$result->id ?>"><?php echo $result->site_name ?></a>
				<div Class="actionDiv">
					<a href="javascript:void(0);" onClick="wer_pkEditFromTable(<?php echo $result->id ?>);">
						<span Class="dashicons-before dashicons-edit"></span><?php echo __('Edit', 'wer_pk') ?>
					</a> |
					<a href="javascript:void(0);" onClick="wer_pkDeleteFromTable(<?php echo $result->id . ',' . '\'\',\' ' . __('Are you sure?', 'wer_pk') . '\''; ?>);">
						<span Class="dashicons-before dashicons-trash"></span><?php echo __('Delete', 'wer_pk') ?>
					</a>
				</div>
			</td>
			<td><?php echo $result->site_size ?></td>
			<td><?php echo $result->site_location ?></td>
			<td><?php echo $result->start_date ?></td>
			<td><?php echo $result->status ? __('Delete', 'On Hold') : __('In Progress', 'wer_pk') ?></td>
		</tr>
		<?php } ?>
